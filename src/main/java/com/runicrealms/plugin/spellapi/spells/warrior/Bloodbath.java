...
public class Bloodbath extends Spell implements AttributeSpell {
    ...
    public Bloodbath() {
        ...
        this.setDescription("Hitting an enemy with &aCleave&7 heals✦ you for " +
                ...
                "Max one target per swing of &aCleave&7. " +
                "Additionally, you deal " + (this.percent * 100) + "% more damage to &cbleeding &7enemies " +
    ...
    private void onPhysicalDamage(PhysicalDamageEvent event) {
        ...
        // Logic for self-heal on Cleave
        if (event.getSpell() instanceof Cleave && !eventMap.containsKey(event.getPlayer().getUniqueId())) {
        ...
        // Logic for bleed damage amplifier on all damage
        if (this.hasSpellEffect(event.getVictim().getUniqueId(), SpellEffectType.BLEED)) {
