...
public class SkillTreeManager implements Listener, SkillTreeAPI {
    ...
    @Override
    public SkillTreeData getSkillTree(UUID uuid, int slot, SkillTreePosition position, Jedis jedis) {
        return this.loadSkillTreeData(uuid, slot, position, jedis);
    }

    @Override
    public SkillTreeData getSkillTree(UUID uuid, int slot, SkillTreePosition position) {
        return loadSkillTreeData(uuid, slot, position);
    }

    @Override
    public int getSpentPoints(UUID uuid, int slot) {
        int spentPoints = 0;
        SkillTreeData first = RunicCore.getSkillTreeAPI().getSkillTree(uuid, slot, SkillTreePosition.FIRST);
        SkillTreeData second = RunicCore.getSkillTreeAPI().getSkillTree(uuid, slot, SkillTreePosition.SECOND);
        SkillTreeData third = RunicCore.getSkillTreeAPI().getSkillTree(uuid, slot, SkillTreePosition.THIRD);
    ...
    public SkillTreeGUI skillTreeGUI(Player player, SkillTreePosition position) {
        ...
        SkillTreeData skillTreeData = playerSkillTreeMap.get(uuid + ":" + position.getValue());
